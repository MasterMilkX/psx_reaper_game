...
public class TestCycle : MonoBehaviour { //MS Motorcycle test - Marcos Schultz (www.schultzgames.com)
    ...
    void Awake(){
        ...
        cmap.PSX_Controller.MoveUp.performed += ctx => {turnVec[2] = 1;};
        cmap.PSX_Controller.MoveDown.performed += ctx => {turnVec[3] = 1;};
        cmap.PSX_Controller.MoveUp.canceled += ctx => {turnVec[2] = 0;};
        cmap.PSX_Controller.MoveDown.canceled += ctx =>{turnVec[3] = 0;};



        transform.rotation = Quaternion.identity;
        ms_Rigidbody = GetComponent<Rigidbody> ();
        ms_Rigidbody.mass = 100;
    ...
    void FixedUpdate () {
        ...
        verticalInput = 0.0f;
        if(turnVec[0] == 1){horizontalInput = -1;}
        else if(turnVec[1] == 1){horizontalInput = 1;}
        if(turnVec[2] == 1){verticalInput = 1;}
        else if(turnVec[3] == 1){verticalInput = -1;}
